...
static const int LAST_POW_BLOCK = 172800;
...
static const int MODIFIER_INTERVAL_SWITCH = 1; // FPoS Start
...
public:
    ...
    {
        // virax: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // virax: get transaction coin age
...
public:
    ...
    // virax: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // virax: two types of block: proof-of-work or proof-of-stake
    ...
    // virax: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // virax: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // virax: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // virax: block index flags
    enum  
    {
